...
def UserFood(request):
    if request.user.is_authenticated:
        ...
        if check == 1:
            for i in mealsToday:
                totalAmount['weight'] = round(totalAmount['weight'] + i.grams, 2)
                totalAmount['calories'] = round(totalAmount['calories'] + i.consumed_kkal, 2)
                totalAmount['proteins'] = round(totalAmount['proteins'] + i.consumed_proteins, 2)
                totalAmount['fats'] = round(totalAmount['fats'] + i.consumed_fats, 2)
                totalAmount['carbohydrates'] = round(
                    totalAmount['carbohydrates'] + i.consumed_carbohydrates, 2)
...
def Herbs_and_spices(request):
    ...
    if search_query and '*' not in search_query:
        ...
        return render(request, 'food_diary/product_selection_herbs&spices.html', {'error': error,
                                                                                  'value': search_query})
    elif not search_query:

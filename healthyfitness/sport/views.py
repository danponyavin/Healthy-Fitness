...
def AddSport(request) -> HttpResponse:
    if request.user.is_authenticated:
        ...
        if request.POST.get('training_time'):
            ...
            if 0 < int(training_time) < 721:
                ...
                training_info = {'id': training, 'type_training': info[0].type_training,
                                 'kkal': info[0].kkal_in_hour, 'training_time': int(training_time)}
                ...
                user_choice = Training_trecker(id_users=user_id, id_training=training_id,
                                               duration_training_min=training_info['training_time'],
                                               burned_kkal=training_info['kkal'],)
                user_choice.save()
...
def SportDiary(request):
    if request.user.is_authenticated:
        ...
        if search_query and valid_data(search_query):
            ...
            flag = 1
        if flag == 1:
            for i in TrainedToday:
                totalAmount['training_time'] += i.duration_training_min
                totalAmount['kkal'] += i.burned_kkal

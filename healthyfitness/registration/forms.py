...
class RegisterUserForm(UserCreationForm):
    username = CharField(label='Логин', max_length=30, widget=TextInput(attrs={'class': 'form-input',
                                                                               'placeholder': 'Введите логин'}))
    email = EmailField(label='Email', widget=EmailInput(attrs={'class': 'form-input', 'placeholder': 'Введите Email'}))
    password1 = CharField(label='Введите пароль', widget=PasswordInput(attrs={'class': 'form-input',
                                                                              'placeholder': 'Введите пароль'}))
    password2 = CharField(label='Повторите пароль', widget=PasswordInput(attrs={'class': 'form-input',
